...
public class Apriori
{
	...
	public void generate_F1() throws Exception
	{
	    ...
	    while (sc.hasNextLine())
	    {
	  	 ...
	  	 support = (no_of_transaction*support_perc+99)/100;
	...
	public void generateFreqItemSets() throws Exception
	{
	  	...
	  	Vector<Vector<Integer>> curr_item_set = new Vector<Vector<Integer>>();
		...
		while(!old_item_set.isEmpty())
		{
	    	...
	    	while (sc.hasNextLine())
		    {
		    	Vector<Integer> transaction = new Vector<Integer>();
		  	  	...
		  	  	{
		  	  		transaction.addElement(Integer.parseInt(field));
		  	  	}
		  	  	for(int i=0;i<curr_item_set.size();i++)
		  	  	{
		  	  		int cntr_transc=0;int cntr_item=0;
		  	  		int siz_transc=transaction.size();
		  	  		int siz_item_set = curr_item_set.get(i).size();
		  	  		while((cntr_item<siz_item_set)&&(cntr_transc<siz_transc))
		  	  		{
		  	  			if(transaction.get(cntr_transc)==curr_item_set.get(i).get(cntr_item))
		  	  			{
		  	  				cntr_item++;
		  	  			}
		  	  			cntr_transc++;
